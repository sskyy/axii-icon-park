// reference to axii type
/// <reference types="axii" />
...
export interface ISvgIconProps {
    // 包裹当前的图标的tag，默认是span
    tag: Atom<string>|string
    // 当前图标的唯一Id
    id: Atom<string>|string

    // 图标尺寸大小，默认1em
    size: Atom<string|number>|number | string

    // 描边宽度
    strokeWidth: Atom<number>|number

    // 描边端点类型
    strokeLinecap: Atom<StrokeLinecap>|StrokeLinecap

    // 描边连接线类型
    strokeLinejoin: Atom<StrokeLinejoin>|StrokeLinejoin

    // 换肤的颜色数组
    colors: (Atom<string>|string)[]
}
...
const IconPropTypes = {
    tag: PropTypes.atom<string>(),
    size: PropTypes.atom<number | string>(),
    strokeWidth: PropTypes.atom<number>(),
    strokeLinecap: PropTypes.atom<StrokeLinecap>(),
    strokeLinejoin: PropTypes.atom<StrokeLinejoin>(),
    theme: PropTypes.atom<Theme>(),
    fill: PropTypes.any
}


// 包裹后的图标属性
type IIconProps = FixedCompatiblePropsType<typeof IconPropTypes>
type IconInnerProps = PropsType<typeof IconPropTypes>
// 包裹前的图标
type IconRender = (createElement: RenderContext["createSVGElement"], props: ISvgIconProps) => HTMLElement
...
const getColor = (fillColor: Atom<string> | undefined, cfgColor: string): Atom<string>|string => {
    if (fillColor) {
        return fillColor
    }
...
// 属性转换函数
export function IconConverter(id: string, icon: IconInnerProps, config: DeepPartial<IIconConfig>): ISvgIconProps {
    const fill: any[] = Array.isArray(icon.fill) ? icon.fill : [icon.fill]
    const colors: (Atom<string>|string)[] = []
...
type Icon = (props: IIconProps, context: RenderContext ) => HTMLElement

// 图标Wrapper
export function IconWrapper(name: string, rtl: boolean, render: IconRender): Icon {
    const Icon =  (props: IIconProps, { createSVGElement}: RenderContext) => {
        const {
            tag,
            size,
            strokeWidth,
        ...
        return render(createSVGElement, svgProps)
    }
